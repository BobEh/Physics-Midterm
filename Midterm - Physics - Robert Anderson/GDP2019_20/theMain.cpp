...
std::vector<iObject*> g_vec_pAsteroidObjects;
std::vector<iObject*> g_vec_pExplosionObjects;
std::vector<iObject*> g_vec_pDamageObjects;
std::vector<iObject*> g_vec_pLaserObjects;
...
int main(void)
{
	...
	iObject* pSafeParts = pFactory->CreateObject("mesh");
	pSafeParts->setMeshName("safeParts");
	pSafeParts->setFriendlyName("safeParts");	// We use to search 
	pSafeParts->setPositionXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pSafeParts->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pSafeParts->setScale(0.009f);
	pSafeParts->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pSafeParts->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pSafeParts->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pSafeParts->setInverseMass(0.0f);
	pSafeParts->setIsVisible(true);
	pSafeParts->setIsWireframe(false);
	::g_vec_pGameObjects.push_back(pSafeParts);

	iObject* pExposedParts = pFactory->CreateObject("mesh");
	pExposedParts->setMeshName("exposedParts");
	pExposedParts->setFriendlyName("exposedParts");	// We use to search 
	pExposedParts->setPositionXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pExposedParts->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
	pExposedParts->setScale(0.009f);
	pExposedParts->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
	pExposedParts->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
	pExposedParts->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
	pExposedParts->setInverseMass(0.0f);
	pExposedParts->setIsVisible(true);
	pExposedParts->setIsWireframe(false);
	::g_vec_pGameObjects.push_back(pExposedParts);
	...
	glm::mat4 matWorld = calculateWorldMatrix(pExposedParts);

	cMesh transformedMesh;
	pPhsyics->CalculateTransformedMesh(exposedPartsMesh, matWorld, transformedMesh);
	...
	while (!glfwWindowShouldClose(window))
	{
		if (laserActive)
		{
			...
			{
				if (g_vec_pExplosionObjects.at(i)->getScale() < 5.0f)
				{
					g_vec_pExplosionObjects.at(i)->setScale(g_vec_pExplosionObjects.at(i)->getScale() + 0.1f);
				}
				else if (g_vec_pExplosionObjects.at(i)->getScale() > 5.0f)
				{
		...
		else if (!laserActive)
		{
			for (int i = 0; i < g_vec_pExplosionObjects.size(); i++)
			{
				...
				{
					iObject* pDamageSphere = pFactory->CreateObject("sphere");
					pDamageSphere->setMeshName("sphere");
					pDamageSphere->setFriendlyName("explosion_sphere");
					pDamageSphere->setPositionXYZ(g_vec_pExplosionObjects.at(i)->getPositionXYZ());
					pDamageSphere->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
					pDamageSphere->setScale(1.0f);
					pDamageSphere->setDebugColour(glm::vec4(0.2f, 0.2f, 0.2f, 1.0f));
					pDamageSphere->setIsWireframe(true);
					pDamageSphere->setInverseMass(0.0f);			// Sphere won't move
					pDamageSphere->setIsVisible(true);
					g_vec_pDamageObjects.push_back(pDamageSphere);
...
// View matrix
		...
		ssTitle
			<< "X: "
			<< pLightsVec.at(0)->getPositionXYZ().x << ", "
			<< "Y: "
			<< pLightsVec.at(0)->getPositionXYZ().y << ", "
			<< "Z: "
			<< pLightsVec.at(0)->getPositionXYZ().z << " :: "
		<< "Const Atten: "
		<< pLightsVec.at(0)->getConstAtten() << " , "
		<< "Linear Atten: "
		<< pLightsVec.at(0)->getLinearAtten() << " , "
		<< "Quadratic Atten: "
		<< pLightsVec.at(0)->getQuadraticAtten();
		...
		if (time > 10)
		{
			int asteroidType = randInRange(1, 3);
			float positionX = randInRange(-50.0f, 50.0f);
			float positionY = randInRange(100.0f, 150.0f);
			float positionZ = randInRange(-50.0f, 50.0f);
			iObject* newAsteroid = pFactory->CreateObject("sphere");
			if (asteroidType == 1)
			{				
				newAsteroid->setMeshName("asteroid011");
				newAsteroid->setFriendlyName("asteroid011");	// We use to search 
				newAsteroid->setPositionXYZ(glm::vec3(positionX, positionY, positionZ));
				newAsteroid->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
				newAsteroid->setScale(0.02f);
				newAsteroid->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
				newAsteroid->setVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
				newAsteroid->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
				newAsteroid->setInverseMass(1.0f);
				newAsteroid->set_SPHERE_radius(1.0f);
				newAsteroid->setIsVisible(true);
				newAsteroid->setIsWireframe(false);				
			}
			else if (asteroidType == 2)
			{
				newAsteroid->setMeshName("asteroid014");
				newAsteroid->setFriendlyName("asteroid014");	// We use to search 
				newAsteroid->setPositionXYZ(glm::vec3(positionX, positionY, positionZ));
				newAsteroid->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
				newAsteroid->setScale(0.02f);
				newAsteroid->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
			...
			{
				newAsteroid->setMeshName("asteroid015");
				newAsteroid->setFriendlyName("asteroid015");	// We use to search 
				newAsteroid->setPositionXYZ(glm::vec3(positionX, positionY, positionZ));
				newAsteroid->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
				newAsteroid->setScale(0.02f);
				newAsteroid->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
			...
			::g_vec_pAsteroidObjects.push_back(newAsteroid);
			time = 1;
		}
		...
		for (int index = 0; index != ::g_vec_pAsteroidObjects.size(); index++)
		{
			glm::mat4 matModel = glm::mat4(1.0f);

			iObject* pCurrentObject = ::g_vec_pAsteroidObjects[index];
		...
		for (int index = 0; index != ::g_vec_pDamageObjects.size(); index++)
		{
			glm::mat4 matModel = glm::mat4(1.0f);

			iObject* pCurrentObject = ::g_vec_pDamageObjects[index];

			DrawObject(matModel, pCurrentObject,
				shaderProgID, pTheVAOManager);

		}//for (int index...
		for (int index = 0; index != ::g_vec_pLaserObjects.size(); index++)
		{
			glm::mat4 matModel = glm::mat4(1.0f);

			iObject* pCurrentObject = ::g_vec_pLaserObjects[index];
		...
		pPhsyics->IntegrationStep(::g_vec_pAsteroidObjects, 0.03f);

		if (laserActive)
		{
			for (int i = 0; i < g_vec_pAsteroidObjects.size(); i++)
			{
				if (g_vec_pAsteroidObjects.at(i)->getPositionXYZ().y < 35.0f)
				{
					float positionX = g_vec_pAsteroidObjects.at(i)->getPositionXYZ().x / 35;
					float positionY = g_vec_pAsteroidObjects.at(i)->getPositionXYZ().y / 35;
					float positionZ = g_vec_pAsteroidObjects.at(i)->getPositionXYZ().z / 35;
					for (int j = 0; j < 35; j++)
					{
						iObject* pLaserSphere = pFactory->CreateObject("mesh");
						pLaserSphere->setMeshName("sphere");
						pLaserSphere->setFriendlyName("Sphere#1");	// We use to search 
						pLaserSphere->setPositionXYZ(glm::vec3(positionX * j, positionY * j, positionZ * j));
						pLaserSphere->setScale(1.0f);
						//pLaserSphere->setObjectColourRGBA(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
						//pLaserSphere->setDiffuseColour(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
						pLaserSphere->setDebugColour(glm::vec4(0.0f, 1.0f, 0.0f, 1.0f));
						pLaserSphere->setVelocity(glm::vec3(0.0f, 3.0f, 0.0f));
						pLaserSphere->setAccel(glm::vec3(0.0f, 0.0f, 0.0f));
						pLaserSphere->setInverseMass(0.0f);
						pLaserSphere->setIsVisible(true);
						pLaserSphere->setIsWireframe(true);
						::g_vec_pLaserObjects.push_back(pLaserSphere);
					}
					iObject* pExplosionSphere = pFactory->CreateObject("sphere");
					pExplosionSphere->setMeshName("sphere");
					pExplosionSphere->setFriendlyName("explosion_sphere");
					pExplosionSphere->setPositionXYZ(g_vec_pAsteroidObjects.at(i)->getPositionXYZ());
					pExplosionSphere->setDebugColour(glm::vec4(1.0f, 0.0f, 0.0f, 1.0f));
					pExplosionSphere->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
					pExplosionSphere->setScale(1.0f);
					pExplosionSphere->setIsWireframe(true);
					pExplosionSphere->setInverseMass(0.0f);			// Sphere won't move
					pExplosionSphere->setIsVisible(true);
					g_vec_pExplosionObjects.push_back(pExplosionSphere);

					g_vec_pAsteroidObjects.erase(g_vec_pAsteroidObjects.begin() + i);
				}
			}

			if (g_vec_pLaserObjects.size() > 0 && laserTime > 20)
			{
				g_vec_pLaserObjects.clear();
				laserTime = 1;
			}
		...
		else if (!laserActive)
		{
			...
			for (int j = 0; j < ::g_vec_pAsteroidObjects.size(); j++)
			{
				...
				if (g_vec_pAsteroidObjects.at(j)->getPositionXYZ().y < -100.0f)
				{
					g_vec_pAsteroidObjects.erase(g_vec_pAsteroidObjects.begin() + j);
				}

				glm::vec3 closestPoint = glm::vec3(0.0f, 0.0f, 0.0f);
				cPhysics::sPhysicsTriangle closestTriangle;

				pPhsyics->GetClosestTriangleToPoint(g_vec_pAsteroidObjects.at(j)->getPositionXYZ(), transformedMesh, closestPoint, closestTriangle);
				...
				float distance = glm::length(g_vec_pAsteroidObjects.at(j)->getPositionXYZ() - closestPoint);

				if (distance <= g_vec_pAsteroidObjects.at(j)->get_SPHERE_radius())
				{
					//if (k == 0)
					//{
					//	g_vec_pBallObjects.at(j)->inverseMass = 0.0f;
					...
					pExplosionSphere->setMeshName("sphere");
					pExplosionSphere->setFriendlyName("explosion_sphere");
					pExplosionSphere->setPositionXYZ(closestPoint);
					pExplosionSphere->setRotationXYZ(glm::vec3(0.0f, 0.0f, 0.0f));
					pExplosionSphere->setScale(1.0f);
					pExplosionSphere->setDebugColour(glm::vec4(1.0f, 0.0f, 0.0f, 1.0f));
					pExplosionSphere->setIsWireframe(true);
				...
				pPhsyics->TestForCollisions(::g_vec_pAsteroidObjects);
				...
				glm::vec3 CentreToClosestPoint = g_vec_pAsteroidObjects.at(j)->getPositionXYZ() - closestPoint;

				// Direction that ball is going is normalized velocity
				glm::vec3 directionBall = glm::normalize(g_vec_pAsteroidObjects.at(j)->getVelocity());	// 1.0f
				...
				pDebugRenderer->addLine(g_vec_pAsteroidObjects.at(j)->getPositionXYZ(),
					closestPoint,
